...
class Obat extends CI_Controller{
	...
	function index(){
		...
		if (!isset($_SESSION['username'])) {
		    ...
		    $this->db->query("INSERT INTO data_akses(fid_user,fitur) VALUES('".$_SESSION['id']."','Pengingat Obat')");
	...
	function insert(){
            ...
            $jam = $_POST['jam'];
                 ...
                 $sql="INSERT INTO data_obat(fid_user,nama_obat,jam) VALUES('$fid_user','$nama_obat','$jam')";
                 $this->db->query($sql);
...
$objWriter->save('php://output');
	...
	function delete(){
		...
		function cek(){
		$fid_obat = $this->uri->segment(3);
		$tanggal = $this->uri->segment(4);

		     
		      $sql="INSERT INTO data_minum(fid_obat,tanggal) VALUES('$fid_obat','$tanggal')";
                if($this->db->query($sql)){
                  
                     $this->session->set_flashdata('update', 'Minum obat berhasil !');
                   redirect($this->modul()); 
                }
