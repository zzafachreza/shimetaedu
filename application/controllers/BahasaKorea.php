...
class BahasaKorea extends CI_Controller{
    ...
    public function level($id = null, $lesson_id = null) {
		if (!isset($_SESSION['username'])) {
			...
			$lessons = $this->Lessonkorea_model->get_lessons_by_level($id); // Ambil data pelajaran berdasarkan level_id
			...
			if ($level_data) {
				...
				$this->load->view('header', $data);
				$this->load->view('bahasakorea/level', $data); // Kirim data ke view
				$this->load->view('footer');
			} else {
