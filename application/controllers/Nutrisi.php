...
class Nutrisi extends CI_Controller{
	...
	function index(){
		...
		if (!isset($_SESSION['username'])) {
		    ...
		    $this->db->query("INSERT INTO data_akses(fid_user,fitur) VALUES('".$_SESSION['id']."','Nutrisi')");
	...
	function insert(){
            ...
            $fid_nutrisi  = $_POST['fid_nutrisi'];
            
            print_r($_POST);
            
            $jml = count($fid_nutrisi);
            
            for($i=0;$i<$jml;$i++){
                
                 $sql="INSERT INTO data_makan(fid_user,tanggal,waktu,fid_nutrisi) VALUES('$fid_user','$tanggal','$waktu','".$fid_nutrisi[$i]."')";
                 $this->db->query($sql);
            }
...
$objWriter->save('php://output');
	...
	function delete(){
		$id = $this->uri->segment(3);
		      ...
		      $sql="DELETE FROM data_makan WHERE id_makan='$id'";
                if($this->db->query($sql)){
